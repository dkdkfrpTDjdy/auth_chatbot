# -*- coding: utf-8 -*-
"""
IAM 최신 요청 파일 전처리 스크립트 (v3)

주요 변경점
1) auth_name == 'PRS' 인 데이터는 전 구간에서 제거
2) 기존 경로의 index_teams_V1 / index_systems_by_team / index_roles_by_team_sys 와 비교 리포트 출력
3) index_teams_V1 의 팀코드-팀명 매핑을 사용해 팀명을 보강
   - 예: '운영파트' -> 팀코드가 기존 index_teams_V1 에 있으면 '자산관리팀_운영파트' 로 보강
   - 기존에 없는 신규 팀코드는 원본 팀명 그대로 유지
4) 비교는 '새로 생성된 결과' 기준으로 수행
   - 기존 데이터와 merge 하지 않음
   - 즉, 과거 index 는 비교/보강 용도로만 사용
"""
...
CONFIG: Dict = {
    "paths": {
        ...
        # 결과물
        "output_workbook": r"D:\works\GEN_AI\auth_chat\auth_chat_2_restore\사용자_조직_권한_메뉴 매핑_20251218_v1.2_updated_v3.xlsx",
        "output_debug_xlsx": r"D:\works\GEN_AI\auth_chat\auth_chat_2_restore\OUTPUT_팀별권한_통합_결과_v3.xlsx",
        "out_base": r"D:\works\GEN_AI\auth_chat\auth_chat_2_restore\public\data\public_data_v3",

        # 기존 운영 경로(비교 + 팀명 보강용)
        "legacy_compare_base": r"D:\works\GEN_AI\auth_chat\auth_chat_2_restore\public\data",
    },
    "source_sheets": {
...
def main() -> None:
    ...
    legacy_compare_base = Path(CONFIG["paths"]["legacy_compare_base"])

    legacy_data = load_legacy_compare_data(legacy_compare_base)
    legacy_team_name_map = legacy_data.get("team_name_map", {})
    ...
    # legacy 팀명 보강
    for name in ["sap_users", "ias_users", "mro_users", "srm_users", "eac_users", "team_target_enriched"]:
        pass
    sap_users = enrich_team_name_with_legacy(sap_users, legacy_team_name_map)
    ias_users = enrich_team_name_with_legacy(ias_users, legacy_team_name_map)
    mro_users = enrich_team_name_with_legacy(mro_users, legacy_team_name_map)
    srm_users = enrich_team_name_with_legacy(srm_users, legacy_team_name_map)
    eac_users = enrich_team_name_with_legacy(eac_users, legacy_team_name_map)
    users_all = enrich_team_name_with_legacy(users_all, legacy_team_name_map)
    team_target_enriched = enrich_team_name_with_legacy(team_target_enriched, legacy_team_name_map)
    ...
    users_team_roles = enrich_team_name_with_legacy(users_team_roles, legacy_team_name_map)
    team_roles = build_team_roles_union(team_target_enriched, users_team_roles)
    team_roles = enrich_team_name_with_legacy(team_roles, legacy_team_name_map)
    ...
    team_roles_menu = enrich_team_name_with_legacy(team_roles_menu, legacy_team_name_map)
    ...
    compare_report = "[COMPARE] 기존 index 파일을 찾지 못했습니다. legacy_compare_base 경로를 확인하세요."
    if legacy_data.get("found"):
        compare_report = build_compare_report(legacy_data, outputs_new)
    compare_report_path = write_compare_report(out_base, compare_report)
